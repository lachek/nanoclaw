...
Run setup scripts automatically. Only pause when user action is required (WhatsApp authentication, configuration choices). Scripts live in `.codex/skills/setup/scripts/` and emit structured status blocks to stdout. Verbose logs go to `logs/setup.log`.
...
Run `./.codex/skills/setup/scripts/01-check-environment.sh` and parse the status block.
...
Run `./.codex/skills/setup/scripts/02-install-deps.sh` and parse the status block.
...
Run `./.codex/skills/setup/scripts/03-setup-container.sh --runtime <chosen>` and parse the status block.
...
If HAS_ENV=true from step 1, read `.env` and check if it already has `OPENAI_API_KEY` or `CODEX_API_KEY`. If so, confirm with user: "You already have Codex credentials configured. Want to keep them or reconfigure?" If keeping, skip to step 5.

AskUserQuestion: OpenAI API key vs Codex-specific API key?

**OpenAI API key:** Tell the user to add `OPENAI_API_KEY=<key>` to the `.env` file in the project root, then let you know when done. Once confirmed, verify the `.env` file has the key.
...
**Codex API key:** Tell the user to add `CODEX_API_KEY=<key>` to the `.env` file in the project root, then let you know when done. Once confirmed, verify the `.env` file has the key.
...
- **QR browser:** Run `./.codex/skills/setup/scripts/04-auth-whatsapp.sh --method qr-browser` (Bash timeout: 150000ms)
- **Pairing code:** Ask for phone number first (country code, no + or spaces, e.g. 14155551234). Run `./.codex/skills/setup/scripts/04-auth-whatsapp.sh --method pairing-code --phone NUMBER` (Bash timeout: 150000ms). Display the PAIRING_CODE from the status block with instructions.
- **QR terminal:** Run `./.codex/skills/setup/scripts/04-auth-whatsapp.sh --method qr-terminal`. Tell user to run `cd PROJECT_PATH && npm run auth` in another terminal. Wait for confirmation.
...
AskUserQuestion: What trigger word? (default: Andy). In group chats, messages starting with @TriggerWord go to the agent. In the main channel, no prefix needed.
...
1. Run `./.codex/skills/setup/scripts/05-sync-groups.sh` (Bash timeout: 60000ms)
...
4. Run `./.codex/skills/setup/scripts/05b-list-groups.sh` to get groups (pipe-separated JID|name lines). Do NOT display the output to the user.
...
Run `./.codex/skills/setup/scripts/06-register-channel.sh` with args:
- `--jid "JID"` — from step 7
...
**If no:** Run `./.codex/skills/setup/scripts/07-configure-mounts.sh --empty`
...
`echo '{"allowedRoots":[...],"blockedPatterns":[],"nonMainReadOnly":true}' | ./.codex/skills/setup/scripts/07-configure-mounts.sh`
...
Run `./.codex/skills/setup/scripts/08-setup-service.sh` and parse the status block.
...
Run `./.codex/skills/setup/scripts/09-verify.sh` and parse the status block.
...
- MOUNT_ALLOWLIST=missing → run `./.codex/skills/setup/scripts/07-configure-mounts.sh --empty` to create a default.
...
**Container agent fails ("Codex process exited with code 1"):** Ensure the container runtime is running — start it with the appropriate command for your runtime. Check container logs in `groups/main/logs/container-*.log`.
