...
src/container-runner.ts               container/agent-runner/
    │                                      │
    │ spawns container                      │ runs Codex CLI agent
...
| **Codex sessions** | `~/.codex/projects/` | Codex CLI session history |
...
# CODEX_API_KEY=sk-...        (Codex-specific key)
```
...
**Workaround:** The system extracts only authentication variables (`OPENAI_API_KEY`, `CODEX_API_KEY`) from `.env` and passes them to the container via stdin JSON. Other env vars are not exposed.
...
echo '{}' | docker run -i \
  ...
  -c 'export $(cat /workspace/env-dir/env | xargs); echo "OpenAI: ${#OPENAI_API_KEY} chars, Codex: ${#CODEX_API_KEY} chars"'
```
...
```
/workspace/
├── env-dir/env           # Environment file (OPENAI_API_KEY or CODEX_API_KEY)
...
The agent-runner uses these Codex CLI SDK options:
...
**Important:** `allowDangerouslySkipPermissions: true` is required when using `permissionMode: 'bypassPermissions'`. Without it, Codex CLI exits with code 1.
...
[ -f .env ] && (grep -q "OPENAI_API_KEY=sk-" .env || grep -q "CODEX_API_KEY=sk-" .env) && echo "OK" || echo "MISSING - add OPENAI_API_KEY or CODEX_API_KEY to .env"
